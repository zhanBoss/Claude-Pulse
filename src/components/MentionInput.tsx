        ...
        if (e.key === "Backspace") {
          ...
          if (startContainer === editorRef.current && startOffset > 0) {
            ...
            if (
              ...
              prevChild.classList.contains("mention-tag")
            ) {
              e.preventDefault();
              prevChild.remove();
              updateState();
          ...
          if (mentionActiveRef.current && mentionAtNodeRef.current) {
            if (
              startContainer === mentionAtNodeRef.current &&
              startOffset === mentionAtOffsetRef.current + 1
            ) {
              mentionActiveRef.current = false;
              mentionAtNodeRef.current = null;
              onMentionDismiss?.();
