    ...
    <ReactMarkdown
      ...
      components={{
        code({ node, inline, className, children, ...props }: any) {
          ...
          return !inline && match ? (
            <CodeBlock language={match[1]} value={codeValue} darkMode={darkMode} />
          ) : (
            <code
              style={{
...
const ChatView = (props: ChatViewProps) => {
  ...
  const filteredRealtimeRecords = useMemo(() => {
    ...
    return realtimeRecords.filter(
      r => r.display.toLowerCase().includes(keyword) || r.project.toLowerCase().includes(keyword)
    )
    ...
    (searchText: string) => {
      ...
      if (searchText.trim()) {
        ...
        const hasRealtime = realtimeRecords.some(
          r =>
            r.display.toLowerCase().includes(keyword) || r.project.toLowerCase().includes(keyword)
        )
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
          ...
          <div style={{ textAlign: 'center', padding: '8px 0 4px' }}>
            {hasMoreHistory ? (
              ...
              >
                加载更多（已显示 {visibleHistoryRecords.length} / {filteredHistoryRecords.length}{' '}
                条）
              </Button>
            ) : (
    ...
    >
      ...
      >
        <div style={{ padding: '16px 20px' }}>
          ...
          {/* 列表内容 */}
          <div style={{ maxHeight: 360, overflow: 'auto' }}>{renderPickerContent()}</div>
        </div>
      </ElectronModal>
